...
endogen_map <- tibble(
  Target = c("Aberrant H3C2","Aberrant H3C8","Aberrant H3C11"),
  endogen = c("H3C2","H3C8","H3C11"
  )
)
...
# For the outlier-filtered data, rename the Cq column
# so that both branches have the same structure
if (filt_outlier == T) {
  data_noNA <- data_noNA |>
    rename(median_Cq = Cq)
}
...
endogen_data <- data_noNA |>
  ...
  rename(endogen = Target, endogen_Cq = median_Cq)
...
write_rds(RQ_data, paste0(out_path, gsub('.csv', '_multiple_endogen.RDS',basename(dt_path))))
...
ggsave('20260722_Human_Prolif_vs_Senesc_aberrant_GM5565_histnormal.pdf', p, width = 10, height = 5)

# Statistical analysis
res_GAPDH <- readRDS("20260722_Cq_GM5565_Hist_Prol_Sen_Aberrant_H3_multiple_endogen.RDS")
